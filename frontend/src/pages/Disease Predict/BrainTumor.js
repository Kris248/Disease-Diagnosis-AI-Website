...
const PredictBrainTumorDisease = () => {
  ...
  const handlePredict = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/predict-braintumor-disease', formData, {
        headers: {
    ...
    // Redirect to Google Maps with a BrainTumor specialist search query
    const findDoctors = () => {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          const { latitude, longitude } = position.coords;
          // Construct Google Maps URL with search keyword and current location
          const googleMapsUrl = `https://www.google.com/maps/search/Neurosurgeon+near+me/@${latitude},${longitude},14z`;
  ...
  return (
    <div className="braintumor-page">
      <div className="braintumor-container">
        <h2 className="title">Brain Tumor Prediction🧠</h2>
        ...
        {response && (
          <div className="result-section">
            ...
            <p>
              {response.message.split(response.prediction.class).map((part, index) => (
                <span key={index}>
                  {part}
                  {index < response.message.split(response.prediction.class).length - 1 && (
                    <span className="highlight">{response.prediction.class}</span>
                  )}
                </span>
              ))}
            </p>
...
export default PredictBrainTumorDisease;
